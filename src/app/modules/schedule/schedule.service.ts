...
const getAllFromDB = async (
    ...
    // Only return schedules that are not already assigned to ANY doctor.
    // These are the "global" slots that can later be linked via the doctor-schedule module.
...
const deleteFromDB = async (id: string): Promise<Schedule> => {
    ...
    if (existing.appointment || existing.doctorSchedules.length > 0) {
        throw new ApiError(
            httpStatus.CONFLICT,
            "Schedule is in use and cannot be deleted",
        );
    }
